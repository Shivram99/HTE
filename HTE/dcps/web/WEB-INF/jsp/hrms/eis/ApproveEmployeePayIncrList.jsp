...
<script type="text/javascript">
	...
	function verifyOrder() {
		...
		if (ans) {
			...
			form.setAttribute("action",
					"./hrms.htm?actionFlag=showPayIncDataForNewOrderNo");

			var inputdsgn = document.createElement("input");
			inputdsgn.setAttribute("type", "hidden");
			inputdsgn.setAttribute("name", "orderNo");
			inputdsgn.setAttribute("value", orderNo);

			var inputFlag = document.createElement("input");
			inputFlag.setAttribute("type", "hidden");
			inputFlag.setAttribute("name", "save");
			inputFlag.setAttribute("value", "Verify");

			var inputBillType = document.createElement("input");
			inputBillType.setAttribute("type", "hidden");
			inputBillType.setAttribute("name", "ddoCodeOfOrder");
			inputBillType.setAttribute("value", ddoCode);
	...
	function rejectOrder() {
		...
		if (ans) {
			...
			inputFlag.setAttribute("type", "hidden");
			inputFlag.setAttribute("name", "save");
			inputFlag.setAttribute("value", "Reject");
	...
	function modifyOrder() {
		...
		if (ans) {
			...
			inputFlag.setAttribute("type", "hidden");
			inputFlag.setAttribute("name", "save");
			inputFlag.setAttribute("value", "modify");
	...
	function getHistory(orderNo, locId) {
		...
		var url = './hdiits.htm?actionFlag=getIncPrintReport';

		//&locId='+locId+'&OrderNo='+orderNo;
		...
		var inputFlag = document.createElement("input");
		inputFlag.setAttribute("type", "hidden");
		inputFlag.setAttribute("name", "locId");
		inputFlag.setAttribute("value", locId);
	...
	function filterIncrement() {
		...
		form.setAttribute("method", "POST");
		form.setAttribute("action",
				"./hrms.htm?actionFlag=getIncrementDataForReptDDO");
			...
			<table align="center">
				<tr>
							...
							<c:forEach var="yearList" items="${yearList}">
								<c:choose>
									<c:when test="${currYear==yearList.lookupShortName}">
										<option value="${yearList.lookupShortName}"
											title="${yearList.lookupDesc}" selected="selected">
											<c:out value="${yearList.lookupDesc}" /></option>
									</c:when>
									<c:otherwise>
										<option value="<c:out value="${yearList.lookupShortName}"/>"
											title="${yearList.lookupDesc}">
											<c:out value="${yearList.lookupDesc}" /></option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
					...
					<td colspan="4" align="center"><input id="btnFilter"
						...
						value="Filter" onclick="filterIncrement();" name="btnFilter"
		...
		<table align="right" width="100%">
			<tr>
				...
				<td align="center"><hdiits:button type="button"
						name="RejectOrder" id="RejectOrder" value="Reject Order"
						onclick="rejectOrder();" /></td>
				<%---adeed by roshan
			<td>
				<hdiits:button type="button" name="editOrder" id="editOrder" value="Edit Order" onclick="modifyOrder();"  />
			</td>---%>
				<%---adeed by roshan---%>
				<td><hdiits:button type="button" name="VerifyOrder"
						id="VerifyOrder" value="Verify Order" onclick="verifyOrder();" />
				</td>
			</tr>
		</table>
		<br />
				...
				<td width="100%">
					<table width="100%" id='tab_new_order' align="left"
						...
						<tr>
							...
							<td width="20%" align="left"><hdiits:dateTime
									name="orderDate" mandatory="true"
									validation="txt.isdt,txt.isrequired" captionid="orderDate" />
							</td>
							<td width="20%">&nbsp;</td>
						</tr>
					</table>
				...
				<h1>
					...
					<display:table name="${fixList}" requestURI="" pagesize=""
						...
						<display:column title="" headerClass="datatableheader"
							...
							<c:choose>
								<c:when
									test="${orderList[3] eq 'Verified' || orderList[3] eq 'Rejected'}">
						...
						<display:column class="tablecelltext" title="DDO Code"
							headerClass="datatableheader" style="text-align: center;">
		    			${orderList[4]} 
		    			<hdiits:hidden name="ddoCode${i}" id="ddoCode${i}"
								default="${orderList[4]}" />
						</display:column>

						<display:column class="tablecelltext" title="DDO Office"
							headerClass="datatableheader" style="text-align: center;">
		    			${orderList[5]}     
		    		</display:column>


						<display:column class="tablecelltext" title="Increment Order Date"
							headerClass="datatableheader" style="text-align: center;">
		  			${orderList[2]}    
		  			</display:column>
	  				...
	  				${orderList[3]}    
						...
						<display:column title="Action" headerClass="datatableheader"
							style="text-align: center;">
							<a
								href="javascript:getHistory('${orderList[0]}','${orderList[1]}')"><b>Print
									Report</b></a>
						</display:column>
						<display:column headerClass="datatableheader" style="display:none">
							<input type="text" name="disableFlag" id="disableFlag"
								value="${orderList[3]}"></input>
						</display:column>
