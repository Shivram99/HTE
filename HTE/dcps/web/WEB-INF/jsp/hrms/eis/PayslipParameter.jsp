...
<fmt:setBundle basename="resources.Payroll" var="constant"
	scope="request" />
<fmt:setBundle basename="resources.eis.eis_common_lables"
	var="constantVariables" scope="request" />
<fmt:message var="adminLoginID" key="ADMINID" bundle="${constant}"
	scope="request">
</fmt:message>
...
<script>
	...
	function findEmployee() {
		...
		url = uri + '&deptId=' + deptId + '&givenMonth=' + givenMonth
				+ '&givenYear=' + givenYear;
	...
	function getEmpFirstNames() {
		...
		if (deptId == '') {
			...
			url = uri + '&emp_first_name=' + empname + '&deptId=' + deptId
					+ '&givenMonth=' + givenMonth + '&givenYear=' + givenYear;
			var actionf = "findEmpNamePayslip";
			uri = './hrms.htm?actionFlag=' + actionf;
			url = uri + url;
			xmlHttp.onreadystatechange = find_EmpName;
		...
		if (xmlHttp.readyState == complete_state) {
			...
			if (XMLDoc == null) {
				...
				for (var i = 0; i < namesEntries.length; i++) {
					...
					text = namesEntries[i].childNodes[1].firstChild.nodeValue
							+ ' '
							+ namesEntries[i].childNodes[2].firstChild.nodeValue
							+ ' '
							+ namesEntries[i].childNodes[3].firstChild.nodeValue;
	...
	function generatePayslip() {
		...
		var WND_W_MARGIN = 0;
		var WND_H_MARGIN = 30;
		var wndWidth = typeof (screen.availWidth) != "undefined" ? screen.availWidth
				: screen.width - WND_W_MARGIN;
		var wndHeight = typeof (screen.availHeight) != "undefined" ? screen.availHeight
				: screen.height - WND_H_MARGIN;

		var payslipMonth = document.getElementById('selMonth').value;
		var payslipBill = document.getElementById('selBillNo').value;
		var payslipYear = document.getElementById('selYear').value;
		var dsgnId = document.getElementById("DsgnId").value;
		var employeeid = document.getElementById("employeeId").value;

		var form = document.createElement("form");
		form.setAttribute("method", "POST");
		form.setAttribute("action", "./hrms.htm?actionFlag=generatePayslip");
		form.setAttribute("target", "Payslip");

		var inputMonth = document.createElement("input");
		inputMonth.setAttribute("type", "hidden");
		inputMonth.setAttribute("name", "selMonth");
		inputMonth.setAttribute("value", payslipMonth);

		var inputBill = document.createElement("input");
		inputBill.setAttribute("type", "hidden");
		inputBill.setAttribute("name", "selBill");
		inputBill.setAttribute("value", payslipBill);

		var inputYear = document.createElement("input");
		inputYear.setAttribute("type", "hidden");
		inputYear.setAttribute("name", "selYear");
		inputYear.setAttribute("value", payslipYear);

		var inputDsgnId = document.createElement("input");
		inputDsgnId.setAttribute("type", "hidden");
		inputDsgnId.setAttribute("name", "dsgnId");
		inputDsgnId.setAttribute("value", dsgnId);

		var inputEmployeeid = document.createElement("input");
		inputEmployeeid.setAttribute("type", "hi dden");
		inputEmployeeid.setAttribute("name", "employeeid");
		inputEmployeeid.setAttribute("value", employeeid);

		form.appendChild(inputMonth);
		form.appendChild(inputBill);
		form.appendChild(inputYear);
		form.appendChild(inputDsgnId);
		form.appendChild(inputEmployeeid);

		document.body.appendChild(form);

		var emplWindowObj = window.open("", "Payslip",
				"status=yes, toolbar=yes, resizable=1, height=" + wndHeight
						+ ", width=" + wndWidth + ", scrollbars=1");
		form.submit();
		form.remove();
	...
	function getDsgnFromBillNo() {
		...
		url = './hrms.htm?actionFlag=' + actionf + '&billNo=' + billNo
				+ '&payslipMonth=' + payslipMonth + '&payslipYear='
				+ payslipYear;
	...
	<div id="tabmenu">
		<ul id="maintab" class="shadetabs">
			<li class="selected"><a href="#" rel="tcontent1"><b><hdiits:caption
							captionid="Payslip Parameters" caption="Payslip Parameters" /> </b></a></li>
		</ul>
	</div>
			...
			<div align="center">
				<table class="PayslipparaTb">
					<tr style="width: 120px">
						...
						<td align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<hdiits:select name="selMonth" size="1" sort="false"
								caption="Month" id="selMonth" mandatory="true"
						...
						<TD>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b><hdiits:caption
									caption="Year" captionid="Year" /></b></TD>
						<td><hdiits:select name="selYear" size="1" sort="false"
								caption="Year" id="selYear" mandatory="true"
								validation="sel.isrequired">
								<hdiits:option value="-1"> --Select-- </hdiits:option>
					...
					<tr>
						<td><hdiits:caption caption="BillNo" captionid="BillNo"
								bundle="${constantVariables}" /></td>
						<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <select
							name="selBillNo" size="1" id="selBillNo" sort="false"
							captionid="billNo" caption="billNo" validation="sel.isrequired"
							mandatory="true" onchange="getDsgnFromBillNo()"
							style="width: 335px">
								<option value="-1">-------------------Select-------------------</option>
								<c:forEach var="billList" items="${billList}">
									<option value="${billList.billHeadId}"
										title="${billList.billId}"><c:out
											value="${billList.billId}">
										</c:out></option>
								</c:forEach>
					...
					<TR>
						<TD align="left"><fmt:message key="BHM.designation"
								bundle="${commonLables}" /></TD>

						<TD>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <hdiits:select
								captionid="BHM.designation" bundle="${commonLables}" id="DsgnId"
								name="dsgn" sort="false" onchange="getDisableEmployee()">
								<hdiits:option value="-1">-------------------Select-------------------</hdiits:option>
							</hdiits:select>
						</TD>
					...
					<TR>
						<TD>Employee Name</TD>
						<TD align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<hdiits:select name="employeeId" validation="sel.isrequired"
								id="employeeId" size="1" mandatory="false"
				...
				<table>
					...
					<c:choose>
						<c:when test="${adminLoginID eq loginId}">
							<tr>
								<td><b> <hdiits:caption captionid="OT.empDept"
											bundle="${commonLables}"></hdiits:caption></b> <hdiits:select
										name="selDept" size="1" id="selDept" captionid="OT.empDept"
										bundle="${commonLables}" sort="false"
										...
										<c:forEach items="${cmnLocVoList}" var="cmnLocVoList">
											...
											<c:choose>
												<c:when test="${cmnLocVoList.locId==loginLocId}">
													<hdiits:option value="${cmnLocVoList.locId}"
														selected="true"> ${cmnLocVoList.locName}</hdiits:option>
												</c:when>
												<c:otherwise>
							...
							<tr>
								<td><b> <hdiits:caption captionid="Search Name"
											caption="Search Name " />
								</b> <input type="text" name="txtEmpFirstName"
									onkeyup="getEmpFirstNames()" onkeypress="return chkKey(event)" />
								</td>
							</tr>
							<tr>
								<td></td>
							...
							<tr>
								<TD><b><hdiits:caption captionid="Employee Name"
											caption="Employee Name" /> </b> <hdiits:select name="cmbEmpNames"
										validation="sel.isrequired" id="empID" size="10"
						...
						<c:otherwise>
							<hdiits:select style='display:none' name="selDept" size="1"
								id="selDept" captionid="OT.empDept" bundle="${commonLables}">
					...
					<tr>
						<td align="center"><hdiits:button name="btn1"
								value="Generate" caption="Generate" id="btnSubmit1"
								captionid="Generate" onclick="generatePayslip()" type="button" />


							<hdiits:button name="Back" type="button" value="Close"
								onclick="closeForm()" /> <hdiits:button name="Reset"
