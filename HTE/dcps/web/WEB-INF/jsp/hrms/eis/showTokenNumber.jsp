...
<script type="text/javascript"
	src="<c:url value="/script/hrms/eis/Address.js"/>">
<script type="text/javascript" src="script/hod/ps/common.js"></script>
<script type="text/javascript"
	src="<c:url value="/script/common/calendar.js"/>"></script>
<script type="text/javascript" src="script/common/CalendarPopup.js"></script>
<fmt:setBundle basename="resources.eis.eis_common_lables"
	var="adminTokenNumberLabel" scope="request" />
<fmt:setBundle basename="resources.Payroll" var="commonLable"
	scope="request" />
<fmt:message var="payBill" key="paybillTypeId" bundle="${commonLable}"
	scope="request">
</fmt:message>
<fmt:message var="arrearBill" key="arrearbillTypeId"
	bundle="${commonLable}" scope="request">
</fmt:message>
<fmt:message var="supplPaybillFirst" key="supplPaybillFirst"
	bundle="${commonLable}" scope="request">
</fmt:message>
<fmt:message var="supplPaybillSecond" key="supplPaybillSecond"
	bundle="${commonLable}" scope="request">
</fmt:message>
<fmt:message var="supplPaybillThird" key="supplPaybillThird"
	bundle="${commonLable}" scope="request">
</fmt:message>
<fmt:message var="multipleMonthSupplBill" key="multipleMonthSupplBill"
	bundle="${commonLable}" scope="request">
</fmt:message>
<fmt:setBundle basename="resources.onlinebillprep.CommonAlerts"
	var="onlinebillcmnAlerts" scope="application" />
<fmt:message var="created" key="created" bundle="${commonLable}"
	scope="request">
</fmt:message>
<fmt:message var="approved" key="approved" bundle="${commonLable}"
	scope="request">
</fmt:message>
<%--added by vaibhav tyagi: start --%>
<fmt:message var="rejected" key="rejected" bundle="${commonLable}"
	scope="request">
</fmt:message>
<%--added by vaibhav tyagi: start --%>
...
<input type="hidden" name="csrfToken" value="${csrfToken}"/>
	...
	<div id="tcontent1"
		...
		<table width="85%" border="1" bordercolor="black" align="center"
			id="searchTable" rules="">
			<tr>
				<td align="center"
			...
			<tr>
				<td align="center"><hdiits:select name="selMonth" size="1"
						sort="false" caption="Month" id="selMonth"
				...
				<td align="center"><hdiits:select name="selYear" size="1"
						sort="false" caption="Year" id="selYear"
						validation="sel.isrequired">
						<hdiits:option value="">-------Selected--------</hdiits:option>
				...
				<td align="center"><hdiits:select name="billNo" id="billNo"
						size="1" caption="Bill No" sort="false"
						onchange="ShowTokenDetails()">
						...
						<c:forEach items="${billNosList}" var="billNosList">
							<c:choose>
								<c:when test="${billNosList.dcpsDdoBillGroupId == curbill}">
									<hdiits:option value="${billNosList.dcpsDdoBillGroupId}"
										selected="true"> ${billNosList.dcpsDdoBillGroupId}</hdiits:option>
								</c:when>
								<c:otherwise>
				...
				<td align="center"><hdiits:select name="selBillType" size="1"
						sort="false" caption="BillType" id="selType"
						validation="sel.isrequired">
						<hdiits:option value="">-------Selected--------</hdiits:option>
				...
				<td align="center"><hdiits:select name="selBillStatus" size="1"
						sort="false" caption="BillStatus" id="selStatus">
						<hdiits:option value="">-------Selected--------</hdiits:option>
		...
		<center style="width: 100%">
			<hdiits:button name="btn" value="showtokennumber"
				caption="Show Pay bills" style="width: 15%" id="tokenview"
				captionid="Show Token Number" onclick="ShowTokenDetails()"
				type="button" />
			<!-- 	
	&nbsp;	&nbsp;	&nbsp;
...
<hdiits:checkbox name='selectAllChkBox' id='selectAllChkBox' value = "" onclick="selDeselectAll(this)" /> <b>Select All</b>
    ...
    --%>
		...
		--%>
			...
			--%>
							<input name="chkbox" id="chkboxId${i}" type="radio"
								align="middle" align="middle" onclick="chkStatus('${i}')" />
							<hdiits:hidden name="tnrbillno${i}" id="tnrbillnoId${i}"
								default="${row.billGrpid}" />
						</c:otherwise>
					</c:choose>
				...
				<display:column style="text-align: center;font-size:12px;"
					class="tablecelltext" title="Bill Id" headerClass="datatableheader">

					<hdiits:hidden name="billNum${i}" id="billNum${i}"
						default="${row.billGrpid}" />
					<a href="javascript:void(0)" onclick="ShowPayBill(${i})">${row.billGrpid}
				</display:column>

				<display:column style="text-align: center;font-size:12px;"
					class="tablecelltext" title="Bill Description"
					headerClass="datatableheader"> 
		  				${row.billDescip}</display:column>

				<display:column style="text-align: center;font-size:12px;"
					class="tablecelltext" title="Scheme Code"
					headerClass="datatableheader"> 
		  				${row.schemeCode}</display:column>

				<display:column style="text-align: center;font-size:12px;"
					class="tablecelltext" title="Scheme Name"
					headerClass="datatableheader"> 
		  				${row.schemeName}</display:column>

				<%--Added by saurabh--%>
				<display:column style="text-align: center;font-size:12px;"
					class="tablecelltext" title="Sub Scheme Code"
					headerClass="datatableheader"> 
				...
				<display:column style="text-align: center;font-size:12px;"
					class="tablecelltext"
					title='<span>Bill Gross Amount<Font size="3" face="Rupee Foradian"> (<Font size="3" face="Rupee Foradian">`</Font>)</Font> </span>'
					headerClass="datatableheader"> 
				...
				<display:column style="text-align: center;font-size:12px;"
					class="tablecelltext"
					title='<span>Net Amount<Font size="3" face="Rupee Foradian"> (<Font size="3" face="Rupee Foradian">`</Font>)</Font> </span>'
					headerClass="datatableheader"> 
		  				${row.netTotalAmount}</display:column>

				<display:column style="text-align: center;" class="tablecelltext"
					title="Status" headerClass="datatableheader">
					<hdiits:hidden name="billStatus${i}" id="billStatus${i}"
						default="${row.billStatus}" /> 
	...
	&nbsp;	&nbsp;	&nbsp;-->
			<hdiits:button name="btnApprove" value="SubmitBill"
				caption="Voucher Entry" id="btnApprove" captionid="Voucher Entry"
				onclick="chkforCons();" type="button" />
			&nbsp; &nbsp; &nbsp;
			<hdiits:button name="btnDiscard" value="DiscardBill"
				caption="Delete Bill" id="btnDiscard" captionid="Discard Bill"
				onclick="isDeletable()" type="button" />
			&nbsp; &nbsp; &nbsp;
			<hdiits:button name="formSubmitButton" onclick="PaybillparaPage()"
				type="button" caption="Generate Bill" captionid="Generate"
				bundle="${genLables}" />
			&nbsp; &nbsp; &nbsp;
			<hdiits:button name="formForwardButton" id="formForwardButton"
				onclick="forwardToLvlTwo()" type="button" caption="Forward"
				captionid="Forward" bundle="${genLables}" />
		</center>
		<br> <br>
		<table width="100%">
			<tr>
				<td align="left"><a
					href="./hrms.htm?actionFlag=loadBillStatisticsData"
					id="insertOneLink"
					style="height: 50px; width: 100px; text-decoration: none; cursor: default;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
				</td>
				<td align="right"><a
					href="./hrms.htm?actionFlag=NetMismatchScreen"
					id="insertSecondLink"
					style="height: 50px; width: 100px; text-decoration: none; cursor: default;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
				</td>
			</tr>
...
function forwardToLvlTwo() {
    ...
    // Validate inputs
    if (!month || !year || trnbill === "-1" || trnbill === false) {
        alert("Error: Invalid input values.");
        return false;
    }

    var answer = confirm('Do you wish to forward the paybill to Level 2 DDO?');
    if (answer) {
        var uri = 'ifms.htm?actionFlag=fwdBillToLvlTwo';
        ...
        var inputBill = createHiddenInput("billNo", trnbill);
        var inputMonth = createHiddenInput("month", month);
        var inputYear = createHiddenInput("year", year);
        ...
        // Show progress and provide feedback
        showProgressbar("Please wait<br>While forwarding bill(s) ...");
...
function ShowTokenDetails() {
    ...
    var form = document.createElement("form");
    form.setAttribute("method", "POST");
    form.setAttribute("action", "./hdiits.htm?actionFlag=showTokenNumber");
    ...
    inputBill.setAttribute("value", bill);

    var inputBillType = document.createElement("input");
    inputBillType.setAttribute("type", "hidden");
    inputBillType.setAttribute("name", "billtype");
    inputBillType.setAttribute("value", billtype);

    var inputBillStatus = document.createElement("input");
    inputBillStatus.setAttribute("type", "hidden");
    inputBillStatus.setAttribute("name", "billStatus");
    inputBillStatus.setAttribute("value", billStatus);

    form.appendChild(inputMonth);
    form.appendChild(inputYear);
    form.appendChild(inputBill);
    form.appendChild(inputBillType);
    form.appendChild(inputBillStatus);
...
function consolidate(myAjax){
	...
	function openWindow(trnbill,Month,Year)
	{
		...
		var url = "hrms.htm?viewName=ApproveBillWindow";
		
			var form = document.createElement("form");
		    form.setAttribute("method", "POST");
		    form.setAttribute("action", url);

		    var inputTrnBill = document.createElement("input");
		    inputTrnBill.setAttribute("type", "hidden");
		    inputTrnBill.setAttribute("name", "trnbill");
		    inputTrnBill.setAttribute("value", trnbill);
				...
				if (xmlHttp.status == 200)
				{
					...
					var status=paybillMapping[0].childNodes[0].firstChild.nodeValue;    
       				...
       				else if((status == 0)||(status == 2))
       				{
	   				    ...
	   				    var curmonth = "${curmonth}"; // Replace with your logic to get current month if not using a template literal
	   				    var curyear = "${curyear}"; // Replace with your logic to get current year if not using a template literal

	   				    var form = document.createElement("form");
	   				    form.setAttribute("method", "POST");
	   				    form.setAttribute("action", "ifms.htm?actionFlag=discardBill&TokenFlag=YES");

	   				    var inputMonth = document.createElement("input");
	   				 	inputMonth.setAttribute("type", "hidden");
	   					inputMonth.setAttribute("name", "month");
	   					inputMonth.setAttribute("value", curmonth);

	   				    
	   				    var inputYear = document.createElement("input");
	   				 	inputYear.setAttribute("type", "hidden");
	   					inputYear.setAttribute("name", "year");
	   					inputYear.setAttribute("value", curyear);
						
	   					var inputBillNo = document.createElement("input");
	   					inputBillNo.setAttribute("type", "hidden");
	   					inputBillNo.setAttribute("name", "billNum");
	   					inputBillNo.setAttribute("value", billNo);

	   					
	   				    form.appendChild(inputMonth);
	   				    form.appendChild(inputYear);
	   				    form.appendChild(inputBillNo);
...
function getBillNo()
	...
	function ShowPayBill(i) {
	    ...
	    var form = document.createElement("form");
	    form.setAttribute("method", "POST");
	    form.setAttribute("action", "hrms.htm?actionFlag=reportService&reportCode=5000008");

	    var inputAction = document.createElement("input");
	    inputAction.setAttribute("type", "hidden");
	    inputAction.setAttribute("name", "action");
	    inputAction.setAttribute("value", "generateReport");

	    var inputFromParaPage = document.createElement("input");
	    inputFromParaPage.setAttribute("type", "hidden");
	    inputFromParaPage.setAttribute("name", "FromParaPage");
	    inputFromParaPage.setAttribute("value", "TRUE");

	    var inputBackBtn = document.createElement("input");
	    inputBackBtn.setAttribute("type", "hidden");
	    inputBackBtn.setAttribute("name", "backBtn");
	    inputBackBtn.setAttribute("value", "0");

	    var inputBillType = document.createElement("input");
	    inputBillType.setAttribute("type", "hidden");
	    inputBillType.setAttribute("name", "billTypePara");
	    inputBillType.setAttribute("value", billtype);
	    ...
	    var inputBillNo = document.createElement("input");
	    inputBillNo.setAttribute("type", "hidden");
	    inputBillNo.setAttribute("name", "billNo");
	    inputBillNo.setAttribute("value", BillNo);

	    form.appendChild(inputAction);
	    form.appendChild(inputFromParaPage);
	    form.appendChild(inputBackBtn);
	    form.appendChild(inputBillType);
	    form.appendChild(inputMonth);
	    form.appendChild(inputYear);
