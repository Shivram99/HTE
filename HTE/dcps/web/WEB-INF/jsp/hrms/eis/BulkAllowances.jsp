...
<script>
	...
	function selectionForm() {
		...
		if ('${empFlag}' == "true" && '${count}' == 2) {
			...
			url = './hrms.htm?actionFlag=getBulkEmpl&empId=' + empId
					+ '&compoType=' + compoType + '&compoId=' + compoId
...
<script>
	...
	function chkValue() {
		...
		if (document.getElementById("cmpType").value == "") {
			...
			if (document.getElementById("Employee_ID_EmployeeSearch").value == document
					.getElementById("Employee_Name_EmployeeSearch").value) {
			...
			url = './ifms.htm?actionFlag=getBulkEmpl&empId=' + empId
					+ '&compoType=' + compoType + '&compoId=' + compoId;
	...
	function stateChanged_FromCompoType() {
		...
		if (xmlHttp.readyState == complete_state) {
			...
			if (XMLDoc == null) {
				...
				window.status = '';
				var monthEntries = XMLDoc
						.getElementsByTagName('allowDeduc-mapping');
	...
	function getEmployeeFromDesination() {
		...
		if (billGroupid > 0 && dsgn > 0) {
			...
			url = './hrms.htm?actionFlag=getDataForBulkAllowances&dsgn=' + dsgn
					+ '&compoType=' + compoType + '&compoId=' + compoId
	...
	function getEmployeeFromBillgroup() {
		...
		if (billGroupid > 0 && dsgn > 0) {
			//alert('for both only');
			/* url = './hrms.htm?actionFlag=getDataForBulkAllowancesFromBill&billGroupid='
					+ billGroupid
					+ '&compoType='
					+ compoType
					+ '&compoId='
		    ...
		    form.setAttribute("action", "./hrms.htm?actionFlag=getDataForBulkAllowancesFromBill");
		    ...
		    inputSevaarthID.setAttribute("name", "compoType");
		    inputSevaarthID.setAttribute("value", compoType);

		    var inputFlag = document.createElement("input");
		    inputFlag.setAttribute("type", "hidden");
		    inputFlag.setAttribute("name", "compoId");
		    inputFlag.setAttribute("value", compoId);
		    
		    var inputDsgn = document.createElement("input");
		    inputDsgn.setAttribute("type", "hidden");
		    inputDsgn.setAttribute("name", "dsgn");
		    inputDsgn.setAttribute("value", dsgn);

		    // Append the hidden inputs to the form
		    form.appendChild(inputBillType);
		    form.appendChild(inputSevaarthID);
		    form.appendChild(inputFlag);
		    form.appendChild(inputDsgn);
	...
	function getEmployeeFromPayitem() {
		...
		if (rav > 0) {
			...
			var url = './hrms.htm?actionFlag=getBulkEmpl&empId=' + rav
					+ '&compoType=' + compoType + '&compoId=' + compoId;
			//+'&dsgn='+dsgn+'&billGroupid='+billGroupid
	...
	function submitForm() {
		...
		/* var URL = './hrms.htm?actionFlag=insertBulkAllw&dsgn=' + dsgn
				+ '&empFlag=' + empFlag + '&billGroupid=' + billGroupid;
		...
		var form = document.createElement("form");
	    form.setAttribute("method", "POST");
	    form.setAttribute("action", "./hrms.htm?actionFlag=insertBulkAllw");
	    ...
	    var inputFlag = document.createElement("input");
	    inputFlag.setAttribute("type", "hidden");
	    inputFlag.setAttribute("name", "empFlag");
	    inputFlag.setAttribute("value", empFlag);
	...
	function getAmount() {
		...
		var chkAllParent = document
				.getElementsByName("selcheckBoxAllowSelectAll");
	...
	function selDeselectAllowances(obj) {
		...
		if (chkAllParent[0].checked) {
			...
			for (var i = 0; i < counter; i++) {
				...
				var ExistingAmount = document.getElementById('existAmount'
						+ varia).value;
	...
	function deselect() {
		...
		for (var i = 0; i < chkLength; i++) {
			...
			if (chkBoxArr[i].checked == false) {
				...
				// document.getElementById('newAmount'+i).value=0;
				var varaible = eval(i + 1);
				if ((document.getElementById('newAmount' + i).value == document
						.getElementById('amount').value))
					document.getElementById('newAmount' + i).value = document
							.getElementById('existAmount' + varaible).value;
			}
		}
	}
	function diff() {
		...
		for (i = 1; i <= '${size}'; i++) {
			...
			document.getElementById('difference' + empId).value = document
					.getElementById('newAmount' + (i - 1)).value
					- document.getElementById('ExistAmount' + empId).value;
...
<body>
	...
	<div id="tcontent1"
		...
		<hdiits:form name="frmBF" validate="true" method="POST" action=""
			...
			<div>
				<table width="80%" align="center" border="0"
					class="BulkAllwoncesTb2">
					<!-- class added by pratik 04-08-23 -->
					<tr>
						<td colspan="4" style="padding-left: 280px"><p
								style="font-family: verdana; color: red;">Note:Please Select
								Type Of Component and Pay Item First.</p></td>
					...
					<tr>
						...
						<td width="20%" align="justify"><hdiits:select name="cmpType"
								id="cmpType" mandatory="true" onchange="GetPayItems();"
								style="width:150px">
						...
						<td class="Label" width="20%" align="justify"><hdiits:select
								name="payItemCombo" id="payItemCombo" mandatory="true"
								onchange="getEmployeeFromPayitem()" style="width:210px">
					...
					<tr>
						...
						<td class="Label" colspan="" width="20%" align="justify"><select
							name="billcombo" id="billcombo"
								...
								<option>
									<c:forEach items="${resValue.billList}" var="billList">
										<option value="${billList.dcpsDdoBillGroupId}"
											title="${billList.dcpsDdoBillDescription}">${billList.dcpsDdoBillDescription}</option>
									</c:forEach>
						</select></td>
						<td width="30%" align="justify"></td>
					...
					<tr>
						...
						<td class="Label" width="20%" align="justify"><hdiits:select
								name="dsgnCombo" id="dsgnCombo" mandatory="false"
								onchange="getEmployeeFromDesination()" style="width:225px">
								<hdiits:option value="">-----------Select-----------</hdiits:option>
			...
			<table width="70%" align="center" border="0">
				<tr>
					<td width="46%"><b><hdiits:caption
								captionid="bulk.selectChkBox" bundle="${commonLables}" /></b><Font
						size="3" face='Rupee Foradian'> (<Font size="3"
							face='Rupee Foradian'>`</Font>)</td>
		...
		<hdiits:form name="newBulkForm" validate="true" method="POST"
			...
			<table width="80%" height="30%" align="center" cellpadding="0"
				cellspacing="0" style="border-collapse: separate;">
				<tr>
					<td height="30%" width="80%" valign="top">
						<div
							style="height: 300px; width: 100%; overflow: auto; margin: 0px; padding: 0px;">
							<table
								style="width: 98%; border: single; overflow-y: auto; overflow-x: hidden;"
								...
								<tr>
									<th
										...
										width="30%" align="center"><b>Employee Name<b></th>
									<th
										style="background-color: #F7E7D7; color: rgb(202, 97, 12); font-size: small; font-style: normal; font-weight: bold"
										width="15%" align="center"><b>Existing Amount <Font
											size="3" face='Rupee Foradian'> (<Font size="3"
												face='Rupee Foradian'>`</Font>)
										</Font></b></th>
									<th
										style="background-color: #F7E7D7; color: rgb(202, 97, 12); font-size: small; font-style: normal; font-weight: bold"
										width="15%" align="center"><b>New Amount <Font
											size="3" face='Rupee Foradian'> (<Font size="3"
												face='Rupee Foradian'>`</Font>)
										</Font></b></th>
									<th
										style="background-color: #F7E7D7; color: rgb(202, 97, 12); font-size: small; font-style: normal; font-weight: bold"
										width="15%" align="center"><b>Difference <Font
											size="3" face='Rupee Foradian'> (<Font size="3"
												face='Rupee Foradian'>`</Font>)
										</Font></b></th>
									<th
										style="background-color: #F7E7D7; color: rgb(202, 97, 12); font-size: small; font-style: normal; font-weight: bold;"
										width="5%" align="center"><b><font color="black">
												<hdiits:checkbox id="selcheckBoxAllowSelectAll"
													name="selcheckBoxAllowSelectAll" value="1"
								...
								<c:forEach var="row" items="${list}">
									...
									<tr>
										<td title="Employee Name"><input type="hidden"
											...
											<input type="hidden" name="edit${row.empId}"
											id="edit${row.empId}" value="${row.edit}" /> <input
											type="hidden" name="hrEmpId${row.empId}"
											id="hrEmpId${row.empId}" value="${row.hrEisEmpId}" /> <c:out
												value="${row.employeeName}"></c:out></td>
										...
										<td title="Existing Amount(Rs)"><input type="text"
											...
											id="ExistAmount${row.empId}" style="text-align: right;"
											name="ExistAmount${row.empId}" value="${row.existingAmount}" />
											<input type="hidden" name="existAmount${counter}"
											id="existAmount${counter}" value="${row.existingAmount}" />
											<c:set var="extAmt" value="${row.existingAmount}"></c:set></td>

										<!-- added by lekhchand seven pc TA 072022 -->

										<c:choose>
											<c:when test="${compoId=='212'}">
												<td><hdiits:hidden default="${row.newAmount}"
														name="tax_name1${srNoDeduction}"
														id="tax_name1${srNoDeduction}" /> <select class="grade"
													style="width: 60%;" title="Grade Obtained"
													id="newAmount${srNoDeduction}" name="newAmount${row.empId}"
													onchange="diff();">
														<option value="0"
															${row.newAmount == '0'  ? 'selected' : ''}>0</option>
														<option value="5400"
															${row.newAmount == 5400 ? 'selected' : ''}>5400</option>
														<option value="2700"
															${row.newAmount == 2700 ? 'selected' : ''}>2700</option>
														<option value="2250"
															${row.newAmount == 2250 ? 'selected' : ''}>2250</option>
														<option value="1350"
															${row.newAmount == 1350 ? 'selected' : ''}>1350</option>
														<option value="1000"
															${row.newAmount == 1000 ? 'selected' : ''}>1000</option>
														<option value="675"
															${row.newAmount ==  675  ? 'selected' : ''}>675</option>
														<option value="4860"
															${row.newAmount == 4860 ? 'selected' : ''}>4860</option>
														<option value="2430"
															${row.newAmount == 2430 ? 'selected' : ''}>2430</option>
														<option value="2025"
															${row.newAmount == 2025 ? 'selected' : ''}>2025</option>
														<option value="1215"
															${row.newAmount == 1215 ? 'selected' : ''}>1215</option>
														<option value="900"
															${row.newAmount == 900  ? 'selected' : ''}>900</option>
														<option value="608"
															${row.newAmount ==  608  ? 'selected' : ''}>608</option>
														<option value="10800"
															${row.newAmount==10800 ? 'selected' : ''}>10800</option>
														<option value="9720"
															${row.newAmount == 9720 ? 'selected' : ''}>9720</option>
												</select> <c:set var="newAmt" value="${row.newAmount}"></c:set></td>
											</c:when>
											<c:otherwise>
												<td title="New Amount(Rs)"><hdiits:hidden
														default="${row.newAmount}"
														name="tax_name1${srNoDeduction}"
														id="tax_name1${srNoDeduction}" /> <input type="text"
													id="newAmount${srNoDeduction}" style="text-align: right;"
													name="newAmount${row.empId}" value="${row.newAmount}"
													onblur="diff()" onkeypress="digitFormat(this)" /> <c:set
								...
								var="newAmt" value="${row.newAmount}"></c:set></td>
								 --%>
										...
										<td title="Select" align="center"><hdiits:checkbox
												id="selcheckBoxAllow${srNoDeduction}"
												name="selcheckBoxAllow" value="1" default="" readonly=""
												onclick="deselect();diff();" /></td>
					...
					--> <input type="hidden" name="hiddenEmpId" id="hiddenEmpId" /> <input
										...
										type="hidden" name="eCode" id="eCode" /> <c:out value="Total"></c:out><Font
										size="3" face='Rupee Foradian'> (<Font size="3"
											face='Rupee Foradian'>`</Font>)
									</Font>
									</td>
									...
									<td><input type="text" id="existAmountTotal"
										style="text-align: right;" name="existAmountTotal"
										value="${eTotal}" disabled="disabled" /></td>

									<td><input type="text" id="newAmountTotal"
										style="text-align: right;" name="newAmountTotal"
										value="${nTotal}" disabled="disabled" /></td>

									<td><input type="text" id="differenceTotal"
										style="text-align: right;" name="differenceTotal"
										value="${dTotal}" disabled="disabled" /></td>
								</tr>
							</table>
						</div>
					</td>
...
-->
	...
	<div align="center">
		<%-- <hdiits:formSubmitButton name="Submit" id="Submit" type="button" caption="Submit" onclick="submitForm()" /> --%>
		<hdiits:button name="Save" style="text-align:center;" id="Save"
			type="button" caption="Save" onclick="submitForm()" />
		<hdiits:button name="btnClose1" style="text-align:center;"
			type="button" id="btnClose1" captionid="eis.close"
			bundle="${commonLables}" onclick="onclosefunction()" />
	</div>
