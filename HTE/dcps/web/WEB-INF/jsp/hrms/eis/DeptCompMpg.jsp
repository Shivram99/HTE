<%@ page
	import="com.tcs.sgv.common.utils.StringUtility,com.tcs.sgv.allowance.valueobject.HrPayAllowTypeMst,com.tcs.sgv.deduction.valueobject.HrPayDeducTypeMst,com.tcs.sgv.eis.valueobject.HrPayEdpCompoMpg,java.util.*"%>
<html>
<head>
<%
try {
...
<c:set var="deducActionList"
	value="${resultObj.resultValue.deducActionList}"></c:set>
<c:set var="DeductionSize"
	value="${resultObj.resultValue.DeductionSize}"></c:set>
...
<c:set var="LoanAdvListSize"
	value="${resultObj.resultValue.LoanAdvListSize}"></c:set>
...
<c:set var="NonGovDeducList"
	value="${resultObj.resultValue.NonGovDeducList}"></c:set>
<c:set var="NonGovDeducSize"
	value="${resultObj.resultValue.NonGovDeducSize}"></c:set>
...
<script type="text/javascript">
	...
	function showAllDed() {
		...
		//var Remarks = document.getElementById('Remarks').value;
			...
			//self.location.href = "hrms.htm?actionFlag=DeptCompMpg&flag=yes&cmbDept="+ cmbDept;
			
			var form = document.createElement("form");
			form.setAttribute("method", "POST");
			form.setAttribute("action", "hrms.htm?actionFlag=DeptCompMpg");
			
			var inputFlag = document.createElement("input");
			inputFlag.setAttribute("type", "hidden");
			inputFlag.setAttribute("name", "flag");
			inputFlag.setAttribute("value", "yes");
			
			var inputcmbDept = document.createElement("input");
			inputcmbDept.setAttribute("type", "hidden");
			inputcmbDept.setAttribute("name", "cmbDept");
			inputcmbDept.setAttribute("value", cmbDept);
			

			form.appendChild(inputFlag);
			form.appendChild(inputcmbDept);
	...
	function ForCheckingValueOnchange() {
		if (xmlHttp.readyState == complete_state) {
			...
			Deductsizeinget = parseInt(Deductsizeinget)
					+ parseInt(nonGovSizeinget);
	...
	function getAllowDeductdata() {
		...
		if (answer) {
			...
			var form = document.createElement("form");
			form.setAttribute("method", "POST");
			form.setAttribute("action", "ifms.htm?actionFlag=InsertDeptCompMpg");
			
			var inputcmbDept = document.createElement("input");
			inputcmbDept.setAttribute("type", "hidden");
			inputcmbDept.setAttribute("name", "applyCmbValue");
			inputcmbDept.setAttribute("value", applyCmbValue);
		...
		for (var i = 1; i <= allowsizeinget; i++) {
			...
			if (applyCmbVal == 2) {
				if (document.getElementById(cheked).checked
						|| document.getElementById(cheked).disabled) {
					if (document.getElementById(cheked).checked) {
						allowForAllEmp += document.getElementById('tax_name2'
								+ i).value
								+ ",";
		...
		for (var i = 1; i <= Deductsizeinget; i++) {
			...
			if (applyCmbVal == 2) {
				if (document.getElementById(cheked).checked
						|| document.getElementById(cheked).disabled) {
					if (document.getElementById(cheked).checked) {
						deductForAllEmp += document.getElementById('tax_name1'
								+ i).value
								+ ",";
	...
	function deselectAllCheckbox() {
		...
		var applyCmbVal = document.DeptCompMPG.applyCmb.options[index].value;
		if (applyCmbVal == 1) {
			url = './hrms.htm?actionFlag=DeptCompMpg&applyCmbValue='
					+ applyCmbVal;
...
<hdiits:form name="DeptCompMPG" validate="true" method="POST" action=""
	encType="multipart/form-data" onload="onloadfunction();">
	...
	<fieldset class="tabstyle">
		...
		<table align="center">
			<tr>
						...
						<c:forEach var="talukaList" items="${talukaList}">
							<c:choose>
								<c:when test="${talukaId==talukaList[0]}">
									<option value="${talukaList[0]}" title="${talukaList[1]}"
										selected="selected">
										<c:out value="${talukaList[1]}" /></option>
								</c:when>
								<c:otherwise>
									<option value="<c:out value="${talukaList[0]}"/>"
										title="${talukaList[1]}">
										<c:out value="${talukaList[1]}" /></option>
								</c:otherwise>
							</c:choose>
			 ...
			 default=""/>
-->
							<input type="text" id="ddoCode" name="ddoCode" maxlength="200"
								value="${ddoSelected}" size="30" />
						</c:when>
						<c:otherwise>
							<input type="text" id="ddoCode" name="ddoCode" maxlength="200"
								size="30" />
			...
			<tr>
				<td colspan="4" align="center"><input id="btnFilter"
					...
					value="Filter" onclick="filterInstituteforDept();" name="btnFilter"
	...
	<div id="tabmenu">
		<ul id="maintab" class="shadetabs">
			<li class="selected"><a href="#" rel="tcontent1"><b><fmt:message
							key="DeptComMpg.DeptCompMpg" bundle="${commonLables}" /></b></a></li>
		</ul>
	</div>
	<br />
	<br />
	...
	<div id="tcontent1"
		...
		<TABLE align="center" width="80%" border="0">
			<tr>
				<!--<td width="10%"></td>
			-->
				<TD align="left" class="Label" width="20%"><b><fmt:message
							key="DeptComMpg.DeptName" bundle="${commonLables}" /></b></TD>
				<td width="10%"><b>:</b></td>
				<TD width="40%"><hdiits:select style="width:90%"
						name="cmbDept" id="cmbDept" size="1" sort="false"
						caption="Department" default="${DeptId}" captionid="Dept"
			...
			<c:if test="${flag eq 'yes'}">
				<TR>
					<!--<td width="5%"></td>
			-->
					<TD align="left" class="Label" width="15%"><b><hdiits:caption
			...
			-->
							<hdiits:dateTime name="WEFDATE" captionid="EmpCompMpg.Date"
								bundle="${commonLables}" mandatory="true"
								validation="txt.isdt,txt.isrequired" minvalue=""
								onblur="vallidateWEF()" default="${WEFDate}"></hdiits:dateTime></b></td>
				...
				<TR>
					<!--<td width="5%"></td>
			-->
					<TD align="left" class="Label" width="15%"><hdiits:caption
							captionid="eis.Remarks" bundle="${commonLables}"
							style="font-style:bold" /></td>
					<td width="10%"><b>:</b></td>
					<td width="40%"><hdiits:textarea rows="3" cols="30"
			...
			<tr id="ApplyTo" style="display: none">
				...
				<td width="10%"><b>:</b></td>
				<TD width="30%"><hdiits:select style="width:90%"
						name="applyCmb" id="applyCmb" size="1" default="-1" sort="false"
						mandatory="true" onchange="deselectAllCheckbox();">
		...
		<c:if test="${flag eq 'yes'}">
			<table width="80%" border=1 rules="groups" id="deduction"
				align="center" bordercolor="#9F5C04"
				style="border-style: solid; border-width: thin">
				<thead>

					<tr bgcolor="#9F5C04">
						<td width="25%" align="left"><b><font color="white"><c:if
										test="${AllowSize ne 0}">
						...
						<td width="25%" align="left"><b><font color="white"><c:if
										test="${DeductionSize ne 0}">
										<hdiits:checkbox name='selectDeductionChkBox'
											id='selectDeductionChkBox' value=""
											onclick="selDeselectDeduction(this)" />
										<b>Deductions</b>
									</c:if> </font></b></td>
						<td width="25%" align="left"><b><font color="white"><c:if
										test="${NonGovDeducSize ne 0}">
										<hdiits:checkbox name='selectDeductionChkBox'
											id='selectDeductionChkBox' value=""
											onclick="selDeselectNonGovDeduction(this)" />
										<b>Non Gov Deduction</b>
									</c:if> </font></b></td>
						<td width="25%" align="left"><b><font color="white"><c:if
										test="${LoanAndAdvList ne 0}">
										<hdiits:checkbox name='selectLoanAdvChkBox'
											id='selectLoanAdvChkBox' value=""
											onclick="selDeselectLaonAdv(this)" />
				...
				<TR>
					...
					<TD width="25%" align="left"><hdiits:hidden
							default="${allowType.allowCode}" name="tax_name2${srNoAllown}"
							id="tax_name2${srNoAllown}" /> <c:choose>
							<c:when
								test="${allowType.allowCode eq 145 or allowType.allowCode eq 9}">
								<hdiits:checkbox id="selcheckBoxAllow${srNoAllown}"
									name="selcheckBoxAllow${srNoAllown}" readonly="" default=""
									value="1" onclick="checkApplyCmb(this.id);" />
							</c:when>
							<c:otherwise>
								<hdiits:checkbox id="selcheckBoxAllow${srNoAllown}"
									name="selcheckBoxAllow${srNoAllown}" value=""
									onclick="checkApplyCmb(this.id);" />
							</c:otherwise>
					...
					<%--<c:choose>
								<c:when test="${allowType.payCommissionId eq deducType.payCommissionId}">
									 --%>
					<TD width="25%" align="left"><hdiits:hidden
							default="${deducType.deducCode}" name="tax_name1${srNoDeduction}"
							id="tax_name1${srNoDeduction}" /> <c:choose>
							<c:when
								test="${deducType.deducCode eq 32 or deducType.deducCode eq 35}">
								<hdiits:checkbox id="selcheckBoxDedu${srNoDeduction}"
									name="selcheckBoxDedu${srNoDeduction}" readonly="" default=""
									value="1" onclick="checkApplyCmb(this.id);" />
							</c:when>
							<c:otherwise>
								<hdiits:checkbox id="selcheckBoxDedu${srNoDeduction}"
									name="selcheckBoxDedu${srNoDeduction}" value=""
									onclick="checkApplyCmb(this.id);" />
							</c:otherwise>
						</c:choose> <span id="span${deducType.deducCode}"></span> <b><c:out
								value="${deducType.deducDisplayName}" /></b> <c:set var="deduccode"
							value="${deduccode+1}" /> <c:set var="srNoDeduction"
							value="${srNoDeduction + 1}"></c:set></TD>
					...
					<TD width="25%" align="left"><hdiits:hidden
							default="${nonGovDeducType.deducCode}"
							name="tax_name1${srNoDeduction}" id="tax_name1${srNoDeduction}" />
						<c:choose>
							<c:when
								test="${nonGovDeducType.deducCode eq 32 or nonGovDeducType.deducCode eq 35}">
								<hdiits:checkbox id="selcheckBoxDedu${srNoDeduction}"
									name="selcheckBoxDedu${srNoDeduction}" readonly="" default=""
									value="1" onclick="checkApplyCmb(this.id);" />
							</c:when>
						...
						</c:choose> <span id="span${nonGovDeducType.deducCode}"></span> <b><c:out
								value="${nonGovDeducType.deducDisplayName}" /></b> <c:set
					...
					<TD width="25%" align="left"><hdiits:hidden
							default="${LoanAndAdvList.typeId}" name="tax_name3${srNoLoanAdv}"
							id="tax_name3${srNoLoanAdv}" /> <hdiits:checkbox
							id="selcheckBoxLoan${srNoLoanAdv}"
							name="selcheckBoxLoan${srNoLoanAdv}" value=""
							onclick="checkApplyCmb(this.id);" /> <b><c:out
								value="${LoanAndAdvList.rltBillTypeEdp.edpDesc}" /></b> <span
						id="span${LoanAndAdvList.typeId}"></span> <c:set var="srNoLoanAdv"
							value="${srNoLoanAdv + 1}"></c:set></TD>
				...
				<hdiits:hidden default="${srNoAllown}" name="srNoAllown"
					id="srNoAllown" />
				<hdiits:hidden default="${srNoLoanAdv}" name="srNoLoanAdv"
					id="srNoLoanAdv" />
			</table>
	...
	<hdiits:hidden default="${LoanAdvListSize}" id="hdnLoanAdvListSize"
		name="hdnLoanAdvListSize"></hdiits:hidden>
	<hdiits:hidden default="${NonGovDeducSize}" id="hdnNonGovDeducSize"
		name="hdnNonGovDeducSize"></hdiits:hidden>
