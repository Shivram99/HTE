...
<hdiits:form name="DCPSChangesDraftForm" id="DCPSForm"
	...
	<fieldset class="tabstyle">
		...
		<input type="hidden" id="hidElementId"
			value="${resValue.hidElementId}" />

		<table align="center" width="90%" border="0">
			<!-- width chage 50 to 90% by pratik 08-08-23   -->
			<tr class="changedraftTr">
				<!-- class added by Pratik 08-08-23  -->
				...
				<td><fmt:message key="CMN.DESIGNATION" bundle="${dcpsLables}"></fmt:message></td>
				<td><select name="cmbDesignation" id="cmbDesignation"
					onChange="">
						<option value="-1"><fmt:message
								key="COMMON.DROPDOWN.SELECT" /></option>
						<c:forEach var="designationVar"
							items="${resValue.lLstDesignation}">
							<c:choose>
								<c:when test="${resValue.DesignationId == designationVar.id}">
									<option value="${designationVar.id}" selected="selected"><c:out
											value="${designationVar.desc}"></c:out></option>
								</c:when>
								<c:otherwise>
									<option value="${designationVar.id}"><c:out
											value="${designationVar.desc}"></c:out></option>
								</c:otherwise>
							</c:choose>
						</c:forEach>
				</select> <input type="hidden" id="User" name="User"
					value="${resValue.UserType}"></td>
			</tr>
			<tr>
				<td colspan="2"></td>
				<!-- added by pratik 08-08-23 -->
				<td id="go" align="right"><br />
				<hdiits:button name="btnGo" id="btnGo" type="button"
						captionid="BTN.GO" bundle="${dcpsLables}"
						onclick="getDraftList();" /></td>

				<td id="back" align="left"><br />
	...
	<c:if test="${resValue.ChangesDraftsList != null}">
		<fieldset class="tabstyle">
			<legend>
				<b><fmt:message key="CMN.CHANGESDRFATS" bundle="${dcpsLables}"></fmt:message></b>
			</legend>
			<div align="center">
				<display:table list="${resValue.ChangesDraftsList}" id="vo"
					requestURI="" export="" style="width:50%" pagesize="10">
					...
					<display:column headerClass="datatableheader" class="oddcentre"
						style="text-align:left" sortable="true" titleKey="CMN.SEVARTHID"
						value="${vo[6]}">
					</display:column>
					...
					<display:column headerClass="datatableheader" class="oddcentre"
						style="text-align:left" sortable="true" titleKey="CMN.EMPLOYEEID"
						value="${vo[3]}">
					</display:column>

					<display:column headerClass="datatableheader" class="oddcentre"
						style="text-align:left" sortable="true"
						titleKey="CMN.CHANGEDETAILS" value="${vo[4]}">
					</display:column>

					<display:column headerClass="datatableheader" class="oddcentre"
						style="text-align:left" sortable="true" titleKey="CMN.DDOCODE"
						value="${vo[7]}">
					</display:column>
