...
<style type="text/css">
fieldset.tabstyle input:not([type='button']), fieldset.tabstyle select,
	fieldset.tabstyle textarea {
	width: 75% !important;
}
</style>
<script type='text/javascript'>
	...
	function CharacterCount(TextArea, FieldToCount, maxCount) {
		...
		if ((txt.length < (maxCount + 1))) {
			document.getElementById(FieldToCount).innerHTML = "<font color='red'>Characters left: "
					+ (maxCount - txt.length) + "</font>";
...
<script>
	function validateContactNo() {
		...
		if (contactNo.charAt(0) != '7' && contactNo.charAt(0) != '8'
				&& contactNo.charAt(0) != '9') {
...
<script>
	function submitNewEntry() {
		...
		if (document.getElementById("ticketTitle").value == ""
				|| document.getElementById("ticketTitle").value == null) {
		...
		if (document.getElementById("ticketDesc").value == ""
				|| document.getElementById("ticketDesc").value == null) {
			alert("Ticket Description cannot be empty");
			return false;
		}
		if (document.getElementById("ticketCred").value == ""
				|| document.getElementById("ticketCred").value == null) {
			alert("Credentials cannot be empty");
			return false;
		}
		if (document.getElementById("ticketContactNo").value == ""
				|| document.getElementById("ticketContactNo").value == null) {
			alert("Contact No cannot be empty");
			return false;
		}

		if (document.getElementById("ticketEmailId").value == ""
				|| document.getElementById("ticketEmailId").value == null) {
	...
	function backToLoad() {
		...
		if (User == 'U') {
			...
			//alert(s);
			if (s != -1) {
				var b = "ifms.htm?actionFlag=loadPRTracker&Flag=" + User
						+ "&status=" + s;
...
<script>
	function updateEntry(flagC) {
		...
		if (User == 'U') {
			...
			var severity = $
			{
				orgTicketMst.severity
			}
			;
			var rootCause = $
			{
				orgTicketMst.rootCause
			}
			;
			if (flagC == 'C') {
				...
				if (answer) {
					...
					url = "ifms.htm?actionFlag=saveNewEntry&flag=Update&ticketId="
							+ tid
							+ "&lStrUser="
							+ User
							+ "&selStatus="
				...
				//url="ifms.htm?actionFlag=saveNewEntry&flag=Update&ticketId="+tid+"&lStrUser="+User+"&selStatus="+status+" &selPriority="+priority+" &ticketRemarks="+remarks+"&selSeverity="+severity+"&selRootCause="+rootCause+"&flagC="+flagC;
				url = "ifms.htm?actionFlag=saveNewEntry&flag=Update&ticketId="
						+ tid + "&lStrUser=" + User + "&selStatus=" + status
						+ " &ticketRemarks=" + remarks + "&selSeverity="
						+ severity + "&selRootCause=" + rootCause + "&flagC="
						+ flagC;
				document.forms[0].action = url;
				document.forms[0].submit();
				showProgressbar('Please wait,Your Request is in progress');
			...
			url = "ifms.htm?actionFlag=saveNewEntry&flag=Update&ticketId="
					+ tid + "&lStrUser=" + User + "&selStatus=" + saveStatus
					+ "  &ticketRemarks=" + remarks + "&selSeverity="
					+ saveSeverity + "&selRootCause=" + saveRootCause;
	...
	<fieldset class="tabstyle">
			...
			<table width="80%" border="0">
				<tr>
					...
					<td width="20%" align="left"><input type="text" id="ticketId"
						name="ticketId" size="40" ${varDisabled}
						value="${orgTicketMst.ticketId}" readonly="readonly"></td>
				</tr>
				<tr>
					<td width="5%" align="left"><fmt:message key="CMN.TICKETITLE"
							bundle="${dcpsLables}"></fmt:message></td>
					<td width="20%" align="left"><input type="text"
						id="ticketTitle" name="ticketTitle" size="40" ${varDisabled}
						value="${orgTicketMst.ticketTitle}"></td>


					<td width="5%" align="left"><fmt:message
							key="CMN.TICKETSCREEN" bundle="${dcpsLables}"></fmt:message></td>
				...
				<tr>
					...
					<td align="left" width="20%"><textarea rows="5" cols="35"
							...
							onkeyup="CharacterCount('ticketDesc','CharCountLabel2',700)"
							${varReadonly}><c:out
					...
					<td align="left" width="20%"><textarea rows="5" cols="35"
							...
							onkeyup="CharacterCount('ticketCred','CharCountLabel1',200)"
							${varDisabled}><c:out
				...
				<tr>
					...
					<td align="left" width="20%"><input type="text"
						id="ticketContactNo" name="ticketContactNo" size="40"
						maxlength="10" onkeypress="digitFormat(this);"
						onblur="validateContactNo();" value="${orgTicketMst.contactNo}"
						${varDisabled}></td>
		...
		<tr><td>&nbsp;</td></tr>--%>
				...
				<tr>
					...
					<td width="5%" align="left"><fmt:message
							key="CMN.TICKETREMARKS" bundle="${dcpsLables}"></fmt:message></td>
					...
					<c:when test="${resValue.lStrUser == 'R'}">
											...
											<c:when test="${orgTicketMst!=null}">
												<c:choose>
													<c:when
														...
														<option value="${lLstSeverity.lookupId}"
						...
						<tr>
									...
									<c:forEach items="${lLstStatus}" var="lLstStatus">
										...
										<c:choose>
											<c:when test="${orgTicketMst!=null}">
												<c:choose>
													<c:when
														test="${orgTicketMst.status == lLstStatus.lookupId}">
							...
							<td width="5%" align="left"><fmt:message key="CMN.TICKETRCA"
									bundle="${dcpsLables}"></fmt:message></td>
							<td align="left"><select name="selRootCause"
								id="selRootCause" style="width: 305px">
									<option value="">-------Selected--------</option>
											...
											<c:when test="${orgTicketMst!=null}">
												<c:choose>
													<c:when
														...
														<option value="${lLstRootCause.lookupId}"
															selected="selected"><c:out
			...
			<td>&nbsp;</td>
				...
				<c:choose>
					<c:when
						test="${resValue.flag == 'Add' && resValue.lStrUser == 'U'}">
						<div align="center">
							<tr>
								<td colspan="2" align="right"><input type="button"
									name="submitEntryButton" class="bigbutton"
									id="submitEntryButton" value="Submit Entry"
									onclick="submitNewEntry(this);" /></td>
								<td align="center"><input type="button" name="backA"
									class="bigbutton" id="backA" value="Back"
						...
						<div align="center">
							<tr>
								<td colspan="2" align="right"><input type="button"
									...
									type="button" value="Update" onclick="updateEntry('A');"
									size="65" /></td>
								<td align="center"><input type="button" name="backU"
									class="bigbutton" id="backU" type="button" value="Back"
									onclick="backToLoad();" size="65" /></td>
								<c:if test="${resValue.lStrUser == 'U'}">
									<td><input type="button" name="closeTicket"
										class="bigbutton" id="closeTicket" type="button"
										value="Close Ticket" onclick="updateEntry('C');" size="65" /></td>
								</c:if>
