...
public class DisplayPendingWorkDaoImpl extends GenericDaoHibernateImpl implements DisplayPendingWorkDao {
	...
	public DisplayPendingWorkDaoImpl(Class<DisplayPendingWorkDaoImpl> class1, SessionFactory sessionFactory) {
		super(class1);
	...
	public int findTotalNumberOfschool(String ddoCode) {
		...
		sb.append("select count(1) from rlt_Zp_ddo_map where status=0 and (REPT_ddo_code=" + ddoCode
				+ " or final_ddo_code=" + ddoCode + ")");
	...
	public int findTotalCountOfPendingOffice(String ddoCode) {
		...
		sb.append("SELECT count(1) FROM MST_DCPS_DDO_OFFICE "
				...
				+ " distinct REPT_DDO_POST_ID from ifms.rlt_zp_ddo_map where rept_ddo_code='" + ddoCode
				+ "' or final_ddo_code='" + ddoCode + "'))");
	...
	public long findCountDraftForms(String ddoCode) {
		...
		StringBuffer sb = new StringBuffer();
		logger.info("---- retrieve usertype------");
		sb.append(" Select count(1) from mst_dcps_emp where zp_status in (0,-1) and form_status=0 and ddo_code="
				+ ddoCode + "");
		logger.info("---- retrieve usertype---" + sb.toString());
		Query query = session.createSQLQuery(sb.toString());
	...
	public List getMissedEmpStatisticsDDOwise(String strDdocode) {
		...
		lSBQuery.append(
				" Select EM.dcps_Emp_Id,EM.EMP_NAME,EM.dob,EM.zp_Status,EM.sentBack_Remarks,EM.gender,EM.ddo_Code,em.form_status,em.reg_status");
	...
	public void sendToLevel1(String strDcpsEmpIds) {
		...
		String query = "update mst_dcps_emp set " + "zp_status=-1, form_status=0, reg_status=0" + " where dcps_emp_id="
				+ strDcpsEmpIds + "";
	...
	public List getApproveEmployeeDDOwise(String strDdocode) {
		...
		lSBQuery.append(
				" Select EM.dcps_Emp_Id,EM.EMP_NAME,EM.sevarth_id,EM.dob,EM.doj,EM.EMP_SERVEND_DT,EM.gender,em.dcps_Or_Gpf");
		lSBQuery.append(" FROM Mst_dcps_emp EM,Rlt_zp_Ddo_Map ZP ");
		lSBQuery.append(" WHERE EM.ddo_Code = ZP.ZP_DDO_CODE  and ZP.ZPLEVEL >= 2 and EM.zp_Status > 4");
		lSBQuery.append(" AND EM.DDO_code =" + strDdocode + "");
		lQuery = session.createSQLQuery(lSBQuery.toString());
	...
	public void deleteSelectedEmployee(String strDcpsEmpIds) {
		logger.info("mstdcpsemp is for update the name is by roshan " + strDcpsEmpIds);
		Session hibSession = getSession();
		String query = "update mst_dcps_emp set " + "ddo_code=null, uid_no=null, reg_status=0" + " where dcps_emp_id="
				+ strDcpsEmpIds + "";
		Query sqlQuery = hibSession.createSQLQuery(query.toString());
		logger.info("query for update status is " + query.toString());
		sqlQuery.executeUpdate();
	...
	public void detachPost(String empId) {
		...
		StringBuilder lSBQuery = new StringBuilder();
		lSBQuery.append(
				"update ORG_USERPOST_RLT set activate_flag=0 where USER_ID = (select user_id from ORG_EMP_MST where EMP_ID  =(select org_emp_mst_id from mst_dcps_emp where dcps_emp_id="
						+ empId + "))");
	...
	public void updateServiceExpiryDate(String empId, String serviceEndDate) {
		...
		Session hibSession = getSession();
		String query = "update mst_dcps_emp set " + " SUPER_ANN_DATE='" + serviceEndDate + "',EMP_SERVEND_DT='"
				+ serviceEndDate + "' " + " where dcps_emp_id=" + empId + "";
		Query sqlQuery = hibSession.createSQLQuery(query.toString());
	...
	public List getMissedEmployeeDDOwise(String strDdocode) {
		...
		Session session = getSession();
		logger.info("hi i Finding Approve  Employee");
		lSBQuery.append(" Select EM.dcps_Emp_Id,EM.EMP_NAME,EM.dob,EM.doj,EM.gender,em.dcps_Or_Gpf");
		lSBQuery.append(" FROM Mst_dcps_emp EM,Rlt_zp_Ddo_Map ZP ");
		lSBQuery.append(" WHERE EM.ddo_Code = ZP.ZP_DDO_CODE  and ZP.ZPLEVEL >= 2 and EM.zp_Status <= 4");
		lSBQuery.append(" AND EM.DDO_code =" + strDdocode + "");
		lQuery = session.createSQLQuery(lSBQuery.toString());
	...
	public List getDDoHistoryDetails(String ddoCode) {
		...
		lSBQuery.append("select sr_no,ddo_code,ddo_name,from_date,to_date from HR_PAY_DDO_HISTORY where DDO_code ="
				+ ddoCode + "");
		lQuery = session.createSQLQuery(lSBQuery.toString());
		ddoList = lQuery.list();
		logger.info("query is" + lQuery.toString());
	...
	public void insertDetails(String ddoCode, String ddoName, String fromDate, String toDate) {
		Session session = getSession();
		logger.info("---- insert hr_pay_ddo_history DAo---");
		StringBuffer sb = new StringBuffer();
		sb.append("insert into ifms.HR_PAY_DDO_HISTORY values ((select max(sr_no)+1 from ifms.HR_PAY_DDO_HISTORY),'"
				+ ddoCode + "','" + ddoName + "',:startingDate,:endDate,sysdate)");
	...
	public void updateDetails(Long SrNo, String updatedDdoName, String updatedFromDate, String updatedTodate) {
		Session session = getSession();
		logger.info("---- update hr_pay_ddo_history DAo---");
		StringBuffer sb = new StringBuffer();
		sb.append("update ifms.HR_PAY_DDO_HISTORY set ddo_name='" + updatedDdoName + "',from_date='" + updatedFromDate
				+ "',to_date=:endDate where sr_no=" + SrNo + " ");
	...
	public void insertNewDetails(String ddoCode, String updatedDdoName, String updatedFromDate, String updatedToDate) {
		Session session = getSession();
		logger.info("---- insert hr_pay_ddo_history DAo---");
		StringBuffer sb = new StringBuffer();
		sb.append("insert into ifms.HR_PAY_DDO_HISTORY values ((select max(sr_no)+1 from ifms.HR_PAY_DDO_HISTORY),'"
				+ ddoCode + "','" + updatedDdoName + "',:startingDate,:endDate,sysdate)");
		logger.info("---- insert hr_pay_ddo_history DAo---" + sb);
		Query query = session.createSQLQuery(sb.toString());
		logger.info("---- query---" + sb);
	...
	public void updateDdoNameInOrgEmpMst(Long lngUserId, String strDdoPersonalName) {

		StringBuilder lSBQuery = new StringBuilder();
		logger.info("hiii i m to update the name in org_emp_mst whose user_id is" + lngUserId);
		Session session = getSession();
		try {
			lSBQuery.append("update org_emp_mst set emp_fname = '" + strDdoPersonalName
					+ "',EMP_MNAME=' ', EMP_LNAME=' ' where user_id=" + lngUserId + " ");
			// lSBQuery.append("in (select user_id from ORG_USERPOST_RLT where post_id ");
			// lSBQuery.append("in (select ddo_post_id from RLT_DCPS_DDO_ASST where
			// ASST_POST_ID = "+lLngDdoAstPostId+"))");
	...
	public List getSchoolList(String strDdocode) {
		...
		lSBQuery.append(
				" Select zp_ddo_code,rept_ddo_code,final_ddo_code,special_ddo_code,status from rlt_zp_ddo_map ");
		lSBQuery.append(" where ((zp_ddo_code='" + strDdocode + "') or (rept_ddo_code='" + strDdocode
				+ "') or (final_ddo_code='" + strDdocode + "') or (special_ddo_code='" + strDdocode
				+ "')) order by zp_ddo_code asc");
		lQuery = session.createSQLQuery(lSBQuery.toString());
	...
	public void deleteSchool(String ddoCode) {
		...
		sb2.append("update ORG_USER_MST set ACTIVATE_FLAG=0 where USER_ID in "
				...
				+ "(SELECT ZP_DDO_POST_ID FROM RLT_ZP_DDO_MAP where ZP_DDO_CODE=" + ddoCode + "))" + " OR USER_NAME = '"
				+ ddoCode + "_AST'");
	...
	public String getEmail(String ddoCode) {
		...
		lSBQuery.append("SELECT coalesce(EMAIL,'Not Available') FROM MST_DCPS_DDO_OFFICE where DDO_CODE = '" + ddoCode
				+ "' and upper(DDO_OFFICE) = 'YES'");
		lQuery = session.createSQLQuery(lSBQuery.toString());
	...
	public String getMobileNo(String ddoCode) {
		...
		lSBQuery.append(
				"SELECT coalesce(cast(TEL_NO2 as varchar),'Not Available') FROM MST_DCPS_DDO_OFFICE where DDO_CODE = '"
						+ ddoCode + "' and upper(DDO_OFFICE) = 'YES'");
		lQuery = session.createSQLQuery(lSBQuery.toString());
	...
	public void updateUDise(String udise, String strDdocode) {
		...
		sb.append("update MST_DCPS_DDO_OFFICE set DICE_CODE = '" + udise + "' where DDO_CODE = '" + strDdocode
				+ "' and upper(DDO_OFFICE) = 'YES'");
	...
	public void updateMobileNo(String mobileNo, String strDdocode) {
		...
		sb.append("update MST_DCPS_DDO_OFFICE set TEL_NO2 = '" + mobileNo + "' where DDO_CODE = '" + strDdocode
				+ "' and upper(DDO_OFFICE) = 'YES'");
	...
	public void updateEmail(String email, String strDdocode) {
		...
		sb.append("update MST_DCPS_DDO_OFFICE set EMAIL = '" + email + "' where DDO_CODE = '" + strDdocode
				+ "' and upper(DDO_OFFICE) = 'YES'");
	...
	public String getInstituteName(String ddoCode) {
		...
		lSBQuery.append("SELECT OFF_NAME FROM MST_DCPS_DDO_OFFICE where DDO_CODE = '" + ddoCode
				+ "' and upper(DDO_OFFICE) = 'YES'");
		lQuery = session.createSQLQuery(lSBQuery.toString());
	...
	public void updateInstituteName(String instituteName, String udise, String lStrDdocode) {
		...
		StringBuffer sb = new StringBuffer();
		sb.append(
				"update MST_DCPS_DDO_OFFICE set OFF_NAME = '" + instituteName + "' where DICE_CODE = '" + udise + "'");
		Query query = session.createSQLQuery(sb.toString());
	...
	public String getTanNo(String strDdocode) {
		...
		lSBQuery.append(
				"SELECT coalesce(TAN_NO,'Not Available') FROM ORG_DDO_MST where  DDO_CODE = '" + strDdocode + "' ");
		lQuery = session.createSQLQuery(lSBQuery.toString());
	...
	public void updateSecretQuestionAndAnswer(String secretQuestion, String secretAnswer, String strDdocode) {

		Session session = getSession();
		logger.info("---- update ORG_USER_MST set secretQuestion:" + secretQuestion + " secretAnswer: " + secretAnswer
				+ " strDdocode:" + strDdocode);
		StringBuffer sb = new StringBuffer();

		if (secretAnswer != null && secretAnswer != "" && secretQuestion != null && secretQuestion != "") {
			sb.append("update ORG_USER_MST set SECRET_QUE_OTHER = '" + secretQuestion + "' , SECRET_ANSWER = '"
					+ secretAnswer + "' where USER_NAME = '" + strDdocode + "'");
			...
			logger.info(
					"---- query to update secretQuestion,secretAnswer ORG_USER_MST--- secretQuestion is NULL and secretAnswer is NULL");
		}
